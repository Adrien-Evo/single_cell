...
buildingHQ_connections <- function(hqListSize, outF, t) { #GLOBAL variable: 1) hap_df.
  ...
  while (HQhap_index_x < length(hapCoveredByLQ)) {
    ...
    tbl_x <- hap_df[which((hap_df[,2] == hapCoveredByLQ[HQhap_index_x] & hap_df[,3] == hapCoveredByLQ[HQhap_index_y]) | (hap_df[,3] == hapCoveredByLQ[HQhap_index_x] & hap_df[,2] == hapCoveredByLQ[HQhap_index_y])),]
    
    if (nrow(tbl_x) > 0) { 
    
        tmp_v <- as.vector(table(hap_df[which((hap_df[,2] == hapCoveredByLQ[HQhap_index_x] & hap_df[,3] == hapCoveredByLQ[HQhap_index_y]) | (hap_df[,3] == hapCoveredByLQ[HQhap_index_x] & hap_df[,2] == hapCoveredByLQ[HQhap_index_y])),5]))
      ...
      hapsExcluded[countExcl] <- HQhap_index_y
      HQhap_index_y <- HQhap_index_y+1
