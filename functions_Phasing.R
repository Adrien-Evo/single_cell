...
excludingDuplicates <- function(tmpUniqueHapList) {
  ...
  while(T) { #merging haplopytes along the list
...
#it takes global variables: hapList, subMatrix, 
creatingLQhaplotypes <- function(LQVarCoord) {
  
  #pos <- 3
  #LQVarCoord <- "23762348"
  #checks how many pairwise comb in the LQ matrix fall involve variants within HQ haplotypes 
  ...
  #hapNb <- 75
  countHapOver <- 1
  for (hapNb in which(overlappingHap >= 2)) {
  ...
  if ((sum(corrPC_one) >= 5  & sum(corrPC_two) >= 5) & (sum(corrPC_one) == sum(corrPC_two))) {
    
    cat(paste("Variant:", LQVarCoord, "kept.", sep=" "), file=outputLQPhasing, sep="\n", append=T)
    if (length(newHap) == 1) {
      
      finalHap <- unique(newHap)
    
      } else if (length(newHap) > 1) {
      
      finalHap <- excludingDuplicates(unique(newHap))[[1]]
